        ...
        ${p.type!=='xy'?`<div style="display:flex;align-items:center;gap:6px;width:100%;padding-top:3px;border-top:1px solid rgba(255,255,255,.05);flex-wrap:wrap">
          ...
          ${p.signals.length>0?`<div style="display:flex;gap:4px;flex-wrap:wrap;margin-left:6px;border-left:1px solid #1e3a5c;padding-left:8px">
            ${p.signals.map((s,si)=>{
              ...
              return `<button onclick="tdaToggleSignalAxis('${p.id}',${si})"
                title="Click to move ${hdr} to ${isR?'Left':'Right'} axis"
                style="padding:1px 6px;background:${isR?'rgba(245,158,11,.15)':'rgba(74,158,255,.12)'};border:1px solid ${isR?'#f59e0b60':'#4a9eff60'};border-radius:3px;color:${col};font-size:8px;cursor:pointer;font-family:var(--mono)">
                ${isR?'R':'L'} ${hdr}
              </button>`;
            }).join('')}
          </div>`:''}
...
function tdaDrawTime(ctx,p,slice,pad,pw,ph,W,H,d,N,i0,i1){
  ...
  const myL = v => pad.t + ph*(1-(v-yminL)/(ymaxL-yminL||1));
  const myR = v => pad.t + ph*(1-(v-yminR)/(ymaxR-yminR||1));
  const my  = s => s.yAxis==='R' ? myR : myL;
  ...
  p.signals.forEach((s,si)=>{
    ...
    const axTag=isR?'[R]':'[L]';
    ctx.fillText(axTag+' '+d.headers[s.ci],lx,ly);
  });
...
window.tdaDrop=function(e,paneId,axis){
  ...
  if(fromBottom<30&&p.type!=='xy'){
    ...
    if(!p.signals.find(s=>s.ci===ci)){
      // Auto-assign to right axis if left already has signals with very different range
      const yAxis = (p.signals.length > 0 && p.rightAxisEnabled) ? 'R' : 'L';
      p.signals.push({ci, yAxis});
    }
...
window.tdaAddPane=function(type){
  ...
  t.panes.push({id,label:type==='xy'?'X/Y Graph':'New Pane',signals:[],xAxis:null,height:200,type:type||'time',yMin_L:null,yMax_L:null,yMin_R:null,yMax_R:null,rightAxisEnabled:false});
  renderDataAnalysis();
};
