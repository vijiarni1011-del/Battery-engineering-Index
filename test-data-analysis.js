...
window.renderDataAnalysis=function(){
  ...
  if(!root) return;
  // Ensure BT tab knows we're in data_analysis mode so propagate doesn't overwrite us
  window._bt_active_cat = 'data_analysis';
...
<style>
/* ─ Layout ─ */
#tda_app{display:flex;flex-direction:column;height:calc(100vh - 130px);min-height:500px;background:#07080b;font-family:'JetBrains Mono',monospace;font-size:11px;overflow:hidden}
/* ─ Topbar ─ */
#tda_topbar{background:#080d18;border-bottom:1px solid #182840;flex-shrink:0;display:flex;flex-direction:column}
#tda_catnav{display:flex;gap:2px;padding:3px 8px;overflow-x:auto;flex-shrink:0}
...
/* ─ Context bar ─ */
#tda_ctx{display:flex;flex-wrap:nowrap;gap:3px;padding:4px 10px;background:rgba(255,255,255,.02);border-bottom:1px solid #182840;align-items:center;overflow-x:auto}
...
/* ─ Toolbar ─ */
#tda_toolbar{display:flex;align-items:center;gap:4px;padding:4px 8px;background:#080d18;border-bottom:1px solid #182840;flex-shrink:0;flex-wrap:wrap}
.tb{padding:3px 8px;background:#0d1626;border:1px solid #182840;border-radius:4px;font-size:10px;color:var(--text2);cursor:pointer;font-family:var(--mono);transition:all .12s;white-space:nowrap}
...
<div id="tda_app">
  ...
  <div id="tda_toolbar">
    ...
    <span style="color:#3a567a;font-size:10px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:500px">
        📄 <b style="color:var(--text2)">${d.fileName}</b>${d.sheetName&&d.sheetName!==d.fileName?` · Sheet: <b style="color:#e879f9">${d.sheetName}</b>`:''} · ${d.nRows.toLocaleString()} rows · ${d.headers.length} ch
      </span>
...
const TDA_COLS_L=['#4a9eff','#ff6b6b','#4ade80','#fbbf24','#a78bfa','#f472b6','#06b6d4','#34d399','#818cf8','#e879f9'];
const TDA_COLS_R=['#ff7b35','#e879f9','#f5c518','#ef4444','#a78bfa','#fb923c','#fbbf24','#f472b6','#facc15','#f87171'];
...
function tdaDrawTime(ctx,p,slice,pad,pw,ph,W,H,d,N,i0,i1){
  ...
  if(t.cursor!=null){
    ...
    const vals=p.signals.map((s,si)=>{const v=+slice[ri]?.[s.ci];if(isNaN(v))return null;const u=tdaDetectUnit(d.headers[s.ci]);const fmt=Math.abs(v)<10?v.toFixed(3):v.toFixed(1);return d.headers[s.ci]+'='+fmt+(u?' '+u:'');}).filter(Boolean);
    ...
    if(ext==='xlsx'||ext==='xls'){
      const loadXlsx=()=>{
        ...
        r.onload=e=>{
          ...
          if(!--pending){window._bt_active_cat='data_analysis';tdaInitPanes();renderDataAnalysis();}
        };
