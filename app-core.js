/* app-core.js - BatteryMIS Core */
(function(){
  // Show splash for 1.8s then fade out and show auth
  setTimeout(function(){
    var s = document.getElementById('__splash');
    if(s){ s.style.opacity='0'; setTimeout(function(){ if(s) s.remove(); }, 650); }
  }, 1800);
})();

(function(){
  try{const r=localStorage.getItem('battmis_session_v1');if(r){const s=JSON.parse(r);if(Date.now()-s.loginTime<8*3600*1000)document.body.style.visibility='';}}catch(e){}
})();
document.addEventListener('DOMContentLoaded',function(){
  if(document.body.style.visibility!=='')return;
  var a=0,p=setInterval(function(){a++;
    if(typeof initAuth==='function'){clearInterval(p);initAuth();}
    else if(a>60){clearInterval(p);document.body.style.visibility='';}
  },80);
});

// ── Global zoom control ──
(function(){
  const ZOOM_KEY = 'battmis_zoom_v1';
  const MIN = 0.6, MAX = 1.0, STEP = 0.05;
  ...
  function applyZoom(z) {
    ...
    const lbl = document.getElementById('zoom_label');
    if (lbl) lbl.textContent = Math.round(zoom*100) + '%';
    // Adjust body padding-top so content stays below fixed topnav at any zoom
    // body.zoom scales coordinates, so 54px CSS = 54*zoom visual pixels
    // topnav is fixed at 54px visual, so padding must be 54/zoom CSS pixels
    document.body.style.paddingTop = Math.round(54 / zoom) + 'px';
    // Adjust subnav fixed top for zoom
    const subnav = document.getElementById('eng-subnav');
    if (subnav) subnav.style.top = Math.round(54 / zoom) + 'px';
  }

  window.zoomIn  = () => applyZoom(zoom + STEP);
  window.zoomOut = () => applyZoom(zoom - STEP);
  window.zoomReset = () => applyZoom(1.0);

  document.addEventListener('DOMContentLoaded', () => {
    applyZoom(zoom);
    // Show subnav since engineering is the default active panel
    const sn = document.getElementById('eng-subnav');
...
// showSec is defined later in the file - assign after load
window.addEventListener('load', function() { window.showSec = showSec; });

(function() {
  /* ── Simulation catalogue ── */
  const SIMS = [
    // Cell Fundamentals 1-10
    { id:1,  cat:'cell', file:'sim-01.html', title:'Cell OCV Measurement', desc:'SoC slider, temperature, load toggle, entropy-corrected OCV curve, chemistry selector', tags:['OCV','SoC','Temperature'], avail:true },
    { id:2,  cat:'cell', file:'sim-02.html', title:'Cell DCIR Measurement', desc:'Pulse discharge, voltage sag, DCIR heatmap (SoC x T), Peukert', tags:['DCIR','Pulse','Heatmap'], avail:true },
    { id:3,  cat:'cell', file:'sim-03.html', title:'OCV vs SoC Curve Mapping', desc:'Charge/discharge curves, hysteresis, dV/dSoC derivative, multi-temperature, all chemistries', tags:['OCV','Hysteresis','dU/dT'], avail:true },
    { id:4,  cat:'cell', file:'sim-04.html', title:'Cell Capacity Measurement (Ah)', desc:'Coulomb counting, C-rate effect, temperature, aging/cycle fade, 80% SoH EoL', tags:['Capacity','SoH','Aging'], avail:true },
    { id:5,  cat:'cell', file:'sim-05.html', title:'C-Rate Effect on Terminal Voltage', desc:'IR drop arrow, all C-rates overlaid, power vs SoC, heat generation, Ragone plot', tags:['C-Rate','Power','Ragone'], avail:true },
    { id:6,  cat:'cell', file:'sim-06.html', title:'Cell Self-Discharge Monitoring', desc:'SEI + shuttle + age mechanism, Arrhenius temp model, 24-month decay, copper dissolution', tags:['Self-discharge','SEI','Aging'], avail:true },
    { id:7,  cat:'cell', file:'sim-07.html', title:'Temperature Effect on Cell Voltage', desc:'3 mechanisms: entropy shift, Rint increase, window compression, voltage surface heatmap', tags:['Temperature','Entropy','Rint'], avail:true },
    { id:8,  cat:'cell', file:'sim-08.html', title:'Cell Voltage Under Load vs No-Load', desc:'ECM transient, R0/R1/R2 breakdown, recovery time, SoC error', tags:['ECM','Thevenin','Recovery'], avail:true },
    { id:9,  cat:'cell', file:'sim-09.html', title:'Hysteresis Between Charge and Discharge', desc:'Thermodynamic hysteresis, mechanism per chemistry, SoC estimation error, energy loss', tags:['Hysteresis','OCV','SoC Error'], avail:true },
    { id:10, cat:'cell', file:'sim-10.html', title:'C-Rate Calculator', desc:'Enter any 2 of cap/I/C-rate. Full reference table, Peukert, heat, time, round-trip efficiency', tags:['C-Rate','Calculator','Efficiency'], avail:true },
    // BMS Core 21-30
    { id:21, cat:'bms', file:'sim-21.html', title:'Overvoltage Protection Cutoff', desc:'Live BMS animation, 5-stage state machine, OVP threshold + hysteresis + delay, overcharge damage model', tags:['OVP','BMS','Protection'], avail:true },
    { id:22, cat:'bms', file:'sim-22.html', title:'Undervoltage Protection Cutoff', desc:'UVP threshold, deep discharge damage model, recovery protocol, delay logic', tags:['UVP','BMS','Deep Discharge'], avail:false },
    { id:23, cat:'bms', file:'sim-23.html', title:'Overcurrent Protection (Charge)', desc:'Charge OCP, I2t model, fuse vs FET comparison, thermal coordination', tags:['OCP','Charge','Fusing'], avail:false },
    { id:24, cat:'bms', file:'sim-24.html', title:'Overcurrent Protection (Discharge)', desc:'Discharge OCP, peak vs continuous, derating curves, contactor coordination', tags:['OCP','Discharge','Derating'], avail:false },
    { id:25, cat:'bms', file:'sim-25.html', title:'Short Circuit Detection and Cutoff', desc:'Hard short vs soft short, detection time < 1ms, current rise dI/dt, MOSFet vs contactor', tags:['Short Circuit','Protection','dI/dt'], avail:false },
    { id:26, cat:'bms', file:'sim-26.html', title:'Cell Balancing - Passive', desc:'Resistive bleed balancing, heat dissipation, equalization time, top vs bottom balancing', tags:['Balancing','Passive','Equalization'], avail:false },
    { id:27, cat:'bms', file:'sim-27.html', title:'Cell Balancing - Active', desc:'Inductive/capacitive transfer, energy efficiency, bi-directional converter concept', tags:['Balancing','Active','Efficiency'], avail:false },
    { id:28, cat:'bms', file:'sim-28.html', title:'Pre-Charge Circuit', desc:'Inrush current limiting, RC time constant, pre-charge relay sequence, capacitor bank sizing', tags:['Pre-charge','Inrush','Relay'], avail:false },
    { id:29, cat:'bms', file:'sim-29.html', title:'Charge/Discharge Enable Logic', desc:'Interlock conditions, state machine, CAN communication, permissive matrix', tags:['Enable Logic','State Machine','CAN'], avail:false },
    { id:30, cat:'bms', file:'sim-30.html', title:'BMS Fault Latch and Reset', desc:'Fault hierarchy, latch logic, reset conditions, persistent fault logging', tags:['Fault','Latch','Reset'], avail:false },
    // Thermal 31-40
    { id:31, cat:'thermal', file:'sim-31.html', title:'Cell Heat Generation Model', desc:'Joule heating + entropic heat, Q = I²R + T×dU/dT×I, heat map vs SoC and C-rate', tags:['Heat','Joule','Entropic'], avail:false },
    { id:32, cat:'thermal', file:'sim-32.html', title:'Thermal Runaway Propagation', desc:'TR trigger model, cell-to-cell propagation, ventilation gas, temperature cascade', tags:['Thermal Runaway','Safety','Propagation'], avail:false },
    { id:33, cat:'thermal', file:'sim-33.html', title:'Cooling Plate Design', desc:'Liquid cooling channel flow, thermal resistance network, delta-T across pack', tags:['Cooling','Liquid','CFD'], avail:false },
    { id:34, cat:'thermal', file:'sim-34.html', title:'BTM System Comparison', desc:'Air vs liquid vs refrigerant cooling - efficiency, weight, cost, thermal uniformity', tags:['BTM','Cooling','Comparison'], avail:false },
    { id:35, cat:'thermal', file:'sim-35.html', title:'Temperature Non-Uniformity', desc:'Gradient across S×P strings, hot cell identification, delta-T threshold', tags:['Non-uniformity','Gradient','SoC Error'], avail:false },
    { id:36, cat:'thermal', file:'sim-36.html', title:'Preheating at Low Temperature', desc:'PTC heater model, energy cost of preheating, DCIR reduction benefit', tags:['Preheat','Low-T','PTC'], avail:false },
    { id:37, cat:'thermal', file:'sim-37.html', title:'Thermal Interface Material', desc:'TIM thickness vs thermal resistance, TIM conductivity comparison, contact pressure', tags:['TIM','Thermal Resistance','Interface'], avail:false },
    { id:38, cat:'thermal', file:'sim-38.html', title:'Heat Pipe & Phase Change', desc:'Heat pipe operation, phase change material (PCM) buffering, latent heat capacity', tags:['Heat Pipe','PCM','Phase Change'], avail:false },
    { id:39, cat:'thermal', file:'sim-39.html', title:'Thermal Model Validation', desc:'Simulated vs measured temperature, lumped thermal model, R-C network', tags:['Validation','R-C Network','Model'], avail:false },
    { id:40, cat:'thermal', file:'sim-40.html', title:'Coolant Flow & Pressure Drop', desc:'Reynolds number, flow regime, pressure drop calculation, pump sizing', tags:['Coolant','Flow','Pressure Drop'], avail:false },
    // Sensing 41-50
    { id:41, cat:'sensing', file:'sim-41.html', title:'Voltage Measurement Accuracy', desc:'ADC resolution, CMV rejection, wire resistance error, flying capacitor principle', tags:['Voltage','ADC','Accuracy'], avail:false },
    { id:42, cat:'sensing', file:'sim-42.html', title:'Current Sensing Methods', desc:'Hall effect vs shunt vs Rogowski, offset drift, temperature coefficient, isolation', tags:['Current Sensor','Hall','Shunt'], avail:false },
    { id:43, cat:'sensing', file:'sim-43.html', title:'Temperature Sensing & Placement', desc:'NTC vs PT100, response time, placement effect on max cell temperature', tags:['NTC','PT100','Temperature'], avail:false },
    { id:44, cat:'sensing', file:'sim-44.html', title:'SoC Estimation Methods', desc:'Coulomb counting drift, OCV lookup, EKF comparison, accuracy vs complexity', tags:['SoC','EKF','Coulomb Counting'], avail:false },
    { id:45, cat:'sensing', file:'sim-45.html', title:'SoH Estimation', desc:'Capacity fade tracking, DCIR growth, EIS-based SoH, calendar vs cycle aging', tags:['SoH','Aging','EIS'], avail:false },
    { id:46, cat:'sensing', file:'sim-46.html', title:'SoP Estimation', desc:'State of Power limits, HPPC-derived Rdc, peak power prediction, thermal derating', tags:['SoP','Power Limits','HPPC'], avail:false },
    { id:47, cat:'sensing', file:'sim-47.html', title:'Isolation Monitoring', desc:'IMD principle, leakage resistance, Y-capacitor effect, fault location', tags:['IMD','Isolation','Leakage'], avail:false },
    { id:48, cat:'sensing', file:'sim-48.html', title:'Cell Imbalance Detection', desc:'Delta-V threshold, imbalance root cause, statistical outlier detection', tags:['Imbalance','Delta-V','Detection'], avail:false },
    { id:49, cat:'sensing', file:'sim-49.html', title:'CAN Communication Basics', desc:'BMS CAN frame structure, message priority, latency, heartbeat monitoring', tags:['CAN','Communication','BMS'], avail:false },
    { id:50, cat:'sensing', file:'sim-50.html', title:'Sensor Fusion for SoX', desc:'Multi-sensor Kalman filter, correlated noise, outlier rejection', tags:['Sensor Fusion','Kalman','SoX'], avail:false },
    // Charging 51-60
    { id:51, cat:'charging', file:'sim-51.html', title:'CC-CV Charging Protocol', desc:'CCCV phases, end condition, charge time vs C-rate, lithium plating risk', tags:['CCCV','Charging','Lithium Plating'], avail:false },
    { id:52, cat:'charging', file:'sim-52.html', title:'Fast Charging Limits', desc:'Max C-rate vs temperature, anode potential, heat generation, degradation per charge', tags:['Fast Charge','Anode','Degradation'], avail:false },
    { id:53, cat:'charging', file:'sim-53.html', title:'AC vs DC Charging', desc:'OBC topology, AC power factor, DC direct input, V2G capability, charging losses', tags:['AC','DC','OBC'], avail:false },
    { id:54, cat:'charging', file:'sim-54.html', title:'Multi-Stage Charging', desc:'Multi-step CC, boost-step, pulse charging effect on capacity and heat', tags:['Multi-Stage','Pulse Charging','Protocol'], avail:false },
    { id:55, cat:'charging', file:'sim-55.html', title:'Charging Temperature Window', desc:'Low-T lithium plating boundary, high-T degradation, adaptive charging', tags:['Temperature','Lithium Plating','Adaptive'], avail:false },
    { id:56, cat:'charging', file:'sim-56.html', title:'Charging Efficiency & Losses', desc:'Ohmic losses, converter losses, round-trip efficiency, heat rejection', tags:['Efficiency','Losses','Round-trip'], avail:false },
    { id:57, cat:'charging', file:'sim-57.html', title:'SoC vs Voltage During Charge', desc:'Relaxation time, OCV correction after charge, measurement artifacts', tags:['SoC','OCV','Relaxation'], avail:false },
    { id:58, cat:'charging', file:'sim-58.html', title:'Regenerative Braking', desc:'Regen power profile, SoC window constraint, derating at high SoC', tags:['Regen','Braking','SoC'], avail:false },
    { id:59, cat:'charging', file:'sim-59.html', title:'Wireless Charging Basics', desc:'Inductive coupling efficiency, coil alignment sensitivity, thermal impact', tags:['Wireless','Inductive','Coupling'], avail:false },
    { id:60, cat:'charging', file:'sim-60.html', title:'Charging Infrastructure Interface', desc:'OCPP protocol basics, smart charging, load management, demand response', tags:['Infrastructure','OCPP','Smart Charging'], avail:false },
    // Protection 61-70
    { id:61, cat:'protection', file:'sim-61.html', title:'Fuse Sizing & I2t', desc:'Pre-arcing energy, fuse vs FET, coordination with upstream protection', tags:['Fuse','I2t','Coordination'], avail:false },
    { id:62, cat:'protection', file:'sim-62.html', title:'Contactor Selection', desc:'Inrush current, contact welding risk, pre-charge coordination, lifetime', tags:['Contactor','Welding','Lifetime'], avail:false },
    { id:63, cat:'protection', file:'sim-63.html', title:'HV Interlock Circuit', desc:'HVIL loop, break detection time, connector sequence, pilot signal', tags:['HVIL','Interlock','HV Safety'], avail:false },
    { id:64, cat:'protection', file:'sim-64.html', title:'MOV & TVS Protection', desc:'Transient voltage suppression, energy absorption, clamping voltage', tags:['MOV','TVS','Transient'], avail:false },
    { id:65, cat:'protection', file:'sim-65.html', title:'Thermal Cutoff Devices', desc:'PTC resettable fuse, bimetallic, CID mechanism, vent pressure', tags:['PTC','Bimetallic','CID'], avail:false },
    { id:66, cat:'protection', file:'sim-66.html', title:'IP Sealing & Ingress', desc:'IP67/69K test conditions, seal integrity, water ingress path', tags:['IP67','Sealing','Ingress'], avail:false },
    { id:67, cat:'protection', file:'sim-67.html', title:'Ground Fault Detection', desc:'Leakage current path, isolation fault location, Y-cap contribution', tags:['Ground Fault','Leakage','Isolation'], avail:false },
    { id:68, cat:'protection', file:'sim-68.html', title:'Arc Flash Energy', desc:'Incident energy calc, arc flash boundary, PPE selection, bus voltage effect', tags:['Arc Flash','PPE','Energy'], avail:false },
    { id:69, cat:'protection', file:'sim-69.html', title:'Emergency Disconnect', desc:'Manual service disconnect, collision disconnect, first responder safety', tags:['Emergency','MSD','Safety'], avail:false },
    { id:70, cat:'protection', file:'sim-70.html', title:'Protection Coordination Study', desc:'Discrimination between layers of protection, time-current curves overlay', tags:['Coordination','TCC','Discrimination'], avail:false },
    // Electrical Characterisation 71-80
    { id:71, cat:'electrical', file:'sim-71.html', title:'EIS - Electrochemical Impedance', desc:'Nyquist plot, Randles circuit, diffusion tail, SEI identification', tags:['EIS','Nyquist','Impedance'], avail:false },
    { id:72, cat:'electrical', file:'sim-72.html', title:'HPPC Test Protocol', desc:'Hybrid pulse power characterization, Rdc extraction, SoC dependence', tags:['HPPC','Rdc','Pulse'], avail:false },
    { id:73, cat:'electrical', file:'sim-73.html', title:'Capacity vs Cycle Count', desc:'NMC vs LFP fade trajectory, knee-point detection, calendar vs cycle', tags:['Capacity Fade','Knee-Point','Aging'], avail:false },
    { id:74, cat:'electrical', file:'sim-74.html', title:'Differential Voltage Analysis', desc:'dV/dQ peak assignment, phase transitions, aging signature identification', tags:['DVA','dV/dQ','Phase Transition'], avail:false },
    { id:75, cat:'electrical', file:'sim-75.html', title:'Incremental Capacity Analysis', desc:'dQ/dV peaks, feature tracking vs cycle, chemistry fingerprint', tags:['ICA','dQ/dV','Chemistry'], avail:false },
    { id:76, cat:'electrical', file:'sim-76.html', title:'Rate Capability Test', desc:'Multi C-rate discharge, capacity retained vs rate, efficiency vs rate', tags:['Rate Capability','C-Rate','Efficiency'], avail:false },
    { id:77, cat:'electrical', file:'sim-77.html', title:'Leakage Current Test', desc:'Self-discharge rate, storage SoC effect, temperature acceleration', tags:['Leakage','Self-Discharge','Storage'], avail:false },
    { id:78, cat:'electrical', file:'sim-78.html', title:'Calendar Aging Model', desc:'Sqrt(t) SEI growth, storage temperature effect, SoC interaction', tags:['Calendar Aging','SEI','sqrt(t)'], avail:false },
    { id:79, cat:'electrical', file:'sim-79.html', title:'Cycle Aging Model', desc:'Rain-flow counting, partial cycle contribution, depth of discharge effect', tags:['Cycle Aging','Rain-Flow','DoD'], avail:false },
    { id:80, cat:'electrical', file:'sim-80.html', title:'Multi-Physics Coupling', desc:'Electrochemical + thermal coupling, local current density, lithium plating map', tags:['Multi-Physics','Coupled','DFN'], avail:false },
    // System Level 81-90
    { id:81, cat:'system', file:'sim-81.html', title:'Pack Energy Sizing', desc:'S×P optimizer, energy-power trade-off, mass/volume constraints, GED target', tags:['Pack Sizing','S×P','GED'], avail:false },
    { id:82, cat:'system', file:'sim-82.html', title:'Vehicle Range Calculator', desc:'EPA/WLTP cycle, auxiliary loads, HVAC draw, range vs temperature', tags:['Range','WLTP','HVAC'], avail:false },
    { id:83, cat:'system', file:'sim-83.html', title:'Drive Cycle Analysis', desc:'Power demand trace, regeneration recovery, RMS current, peak demand', tags:['Drive Cycle','Regen','RMS'], avail:false },
    { id:84, cat:'system', file:'sim-84.html', title:'Pack Voltage Architecture', desc:'400V vs 800V trade-off, cable sizing, charging power, switching losses', tags:['400V','800V','Architecture'], avail:false },
    { id:85, cat:'system', file:'sim-85.html', title:'Inverter-Battery Interface', desc:'DC bus capacitor, ripple current, harmonic content, bearing current risk', tags:['Inverter','Ripple','DC Bus'], avail:false },
    { id:86, cat:'system', file:'sim-86.html', title:'BMS-VCU Communication', desc:'CAN message map, SoC/SoH reporting, torque/power limits, diagnostic UDS', tags:['BMS','VCU','CAN'], avail:false },
    { id:87, cat:'system', file:'sim-87.html', title:'Pack Power Capability', desc:'Continuous vs peak, thermal derating, altitude effect, end-of-life projection', tags:['Power Capability','Derating','EoL'], avail:false },
    { id:88, cat:'system', file:'sim-88.html', title:'Second Life Battery Assessment', desc:'SoH gate criteria, repurposing use case, economic analysis, safety re-assessment', tags:['Second Life','SoH','Repurposing'], avail:false },
    { id:89, cat:'system', file:'sim-89.html', title:'Battery Swap System', desc:'Standardisation requirements, swap time, SoC management, fleet logistics', tags:['Battery Swap','Fleet','Logistics'], avail:false },
    { id:90, cat:'system', file:'sim-90.html', title:'System-Level TCO Analysis', desc:'CapEx vs OpEx, degradation cost, charging cost, residual value', tags:['TCO','CapEx','OpEx'], avail:false },
    // Abuse & Safety 91-100
    { id:91, cat:'abuse', file:'sim-91.html', title:'Overcharge Abuse', desc:'Voltage runaway above 100% SoC, oxygen release, separator melt sequence', tags:['Overcharge','Abuse','Separator'], avail:false },
    { id:92, cat:'abuse', file:'sim-92.html', title:'Over-Discharge Abuse', desc:'Copper dissolution < 2.5V, anode collapse, recovery protocol', tags:['Over-Discharge','Copper','Recovery'], avail:false },
    { id:93, cat:'abuse', file:'sim-93.html', title:'External Short Circuit', desc:'Peak current, fuse response time, cell temperature rise, I2t comparison', tags:['Short Circuit','Fuse','Temperature'], avail:false },
    { id:94, cat:'abuse', file:'sim-94.html', title:'Nail Penetration Test', desc:'Internal short circuit model, thermal runaway trigger, current redistribution', tags:['Nail Penetration','ISC','Thermal Runaway'], avail:false },
    { id:95, cat:'abuse', file:'sim-95.html', title:'Crush Test Response', desc:'Mechanical deformation, separator failure, soft vs hard short progression', tags:['Crush','Mechanical','ISC'], avail:false },
    { id:96, cat:'abuse', file:'sim-96.html', title:'Drop & Vibration Fatigue', desc:'Mechanical stress on tabs, weld failures, capacity loss from vibration cycling', tags:['Drop','Vibration','Fatigue'], avail:false },
    { id:97, cat:'abuse', file:'sim-97.html', title:'Fire & Extreme Heat', desc:'Onset temperature per chemistry, decomposition sequence, gas vent prediction', tags:['Fire','Heat','Decomposition'], avail:false },
    { id:98, cat:'abuse', file:'sim-98.html', title:'Water Immersion', desc:'Leakage path resistance, current in water, shock hazard, seal failure mode', tags:['Immersion','IP','Leakage'], avail:false },
    { id:99, cat:'abuse', file:'sim-99.html', title:'Altitude & Pressure', desc:'Low pressure seal integrity, plasma arc gap reduction, derating at altitude', tags:['Altitude','Pressure','Arc'], avail:false },
    { id:100, cat:'abuse', file:'sim-100.html', title:'EMC Susceptibility', desc:'Conducted/radiated immunity, BMS malfunction threshold, shielding effectiveness', tags:['EMC','Immunity','Shielding'], avail:false },
    // Standards 101-110
    { id:101, cat:'standards', file:'sim-101.html', title:'IEC 62133 - Cell Safety', desc:'Test sequence, pass/fail criteria, cell-level safety requirements mapping', tags:['IEC 62133','Cell Safety','Compliance'], avail:false },
    { id:102, cat:'standards', file:'sim-102.html', title:'UN 38.3 - Transport Testing', desc:'T1-T8 test sequence, altitude, thermal, vibration, shock, short, overcharge', tags:['UN 38.3','Transport','Test Sequence'], avail:false },
    { id:103, cat:'standards', file:'sim-103.html', title:'ISO 26262 - Functional Safety', desc:'ASIL assignment, hazard analysis, safety goals, BMS fault tolerance', tags:['ISO 26262','ASIL','Functional Safety'], avail:false },
    { id:104, cat:'standards', file:'sim-104.html', title:'IEC 62619 - Industrial Battery Safety', desc:'System-level requirements, BMS monitoring functions, installation requirements', tags:['IEC 62619','Industrial','System'], avail:false },
    { id:105, cat:'standards', file:'sim-105.html', title:'GB/T 31485 - China EV Battery', desc:'Chinese mandatory tests, performance and safety, certification pathway', tags:['GB/T','China','EV Battery'], avail:false },
    { id:106, cat:'standards', file:'sim-106.html', title:'UL 9540A - Fire Test', desc:'Full-scale fire test, propagation assessment, mitigation measures scoring', tags:['UL 9540A','Fire Test','Propagation'], avail:false },
    { id:107, cat:'standards', file:'sim-107.html', title:'CISPR 25 - EMC for Vehicles', desc:'Radiated emissions limits, measurement setup, antenna factors, limit lines', tags:['CISPR 25','EMC','Emissions'], avail:false },
    { id:108, cat:'standards', file:'sim-108.html', title:'AIS 038 - India EV Standard', desc:'Indian market type approval, performance and safety alignment with CMVR', tags:['AIS 038','India','Type Approval'], avail:false },
    { id:109, cat:'standards', file:'sim-109.html', title:'EU Battery Regulation 2023', desc:'EU regulation requirements, battery passport, due diligence, recycling targets', tags:['EU Regulation','Battery Passport','Recycling'], avail:false },
    { id:110, cat:'standards', file:'sim-110.html', title:'DVP Plan to Standards Mapping', desc:'Linking DVP test items to applicable standards, traceability matrix', tags:['DVP','Traceability','Standards Map'], avail:false },
  ];
  ...
  let viewerIdx = 0;
  let viewerSims = [];
  const done = JSON.parse(localStorage.getItem('battmis_lab_done') || '[]');

  function markDone(id) {
    if (!done.includes(id)) done.push(id);
    localStorage.setItem('battmis_lab_done', JSON.stringify(done));
    updateStats();
    renderGrid();
  }

  function updateStats() {
    const avail = SIMS.filter(s => s.avail).length;
    document.getElementById('lab-stat-done').textContent = done.length;
    document.getElementById('lab-stat-avail').textContent = avail;
  }

  function labSetCat(cat, el) {
    activeCat = cat;
    document.querySelectorAll('.lab-cat').forEach(c => c.classList.remove('active'));
    if (el) el.classList.add('active');
    renderGrid();
  }

  window.labSetCat = labSetCat;

  function labFilter(q) {
    searchQ = q.toLowerCase();
    renderGrid();
  }
  window.labFilter = labFilter;
  ...
  function renderGrid() {
    ...
    if (!filtered.length) {
      grid.innerHTML = '<div style="grid-column:1/-1;padding:40px;text-align:center;color:#9ca3af;font-size:12px">No simulations found</div>';
      return;
    }

    // Group by category when showing all
    const cats = { cell:'Cell Fundamentals', bms:'BMS Core', thermal:'Thermal Management',
      sensing:'Sensing & Measurement', charging:'Charging Systems', protection:'Protection & Safety',
      electrical:'Electrical Characterisation', system:'System Level', abuse:'Abuse & Safety', standards:'Standards' };

    let html = '';
    if (activeCat === 'all' && !searchQ) {
      const groups = {};
      filtered.forEach(s => { if (!groups[s.cat]) groups[s.cat] = []; groups[s.cat].push(s); });
      Object.keys(groups).forEach(cat => {
        html += '<div class="cat-section-title">' + (cats[cat]||cat) + '</div>';
        groups[cat].forEach((s,i) => { html += simCard(s, filtered.indexOf(s)); });
      });
    } else {
      filtered.forEach((s,i) => { html += simCard(s, i); });
    }

    grid.innerHTML = html;
  }

  function simCard(s, idx) {
    const isDone = done.includes(s.id);
    const soonClass = s.avail ? '' : ' sim-soon';
    const doneBadge = isDone ? '<div class="sim-done-badge">✓</div>' : '';
    const soonBadge = !s.avail ? '<div class="soon-badge">Soon</div>' : '';
    const doneClass = isDone ? ' done' : '';
    const tags = s.tags.slice(0,3).map(t => '<span class="sim-tag">'+t+'</span>').join('');
    const onclick = s.avail ? 'labOpenViewer('+idx+')' : '';
    return '<div class="sim-card'+doneClass+soonClass+'" onclick="'+onclick+'">'+doneBadge+soonBadge+
      '<div class="sim-num">Sim '+String(s.id).padStart(2,'0')+'</div>'+
      '<div class="sim-title">'+s.title+'</div>'+
      '<div class="sim-desc">'+s.desc+'</div>'+
      '<div class="sim-tags">'+tags+'</div></div>';
  }

  window.labOpenViewer = function(idx) {
    viewerIdx = idx;
    const sim = viewerSims[idx];
    if (!sim) return;
    document.getElementById('lab-viewer-title').textContent = 'Sim '+String(sim.id).padStart(2,'0')+' - '+sim.title;
    ...
    document.getElementById('lab-viewer').classList.add('open');
    document.body.style.overflow = 'hidden';
  };

  window.labCloseViewer = function() {
    document.getElementById('lab-viewer').classList.remove('open');
    document.getElementById('lab-viewer-frame').src = 'about:blank';
    document.body.style.overflow = '';
  };

  window.labViewerNav = function(dir) {
    const newIdx = Math.max(0, Math.min(viewerSims.length-1, viewerIdx + dir));
    if (newIdx !== viewerIdx) labOpenViewer(newIdx);
  };

  window.labMarkDone = function() {
    const sim = viewerSims[viewerIdx];
    if (sim) markDone(sim.id);
    document.getElementById('lab-viewer-mark').textContent = '✓ Done!';
    setTimeout(() => { document.getElementById('lab-viewer-mark').textContent = '✓ Mark done'; }, 1500);
  };

  // Keyboard nav
  document.addEventListener('keydown', function(e) {
    if (!document.getElementById('lab-viewer').classList.contains('open')) return;
    if (e.key === 'Escape') window.labCloseViewer();
    if (e.key === 'ArrowRight') window.labViewerNav(1);
    if (e.key === 'ArrowLeft') window.labViewerNav(-1);
  });
  ...
  const observer = new MutationObserver(function(mutations) {
    mutations.forEach(function(m) {
      if (m.target.id === 'panel-learning' && m.target.classList.contains('active')) {
        init();
      }
    });
  });
  ...
  // Also run if already active
  if (panel && panel.classList.contains('active')) init();

  // Expose for sidebar nav trigger
  window.initLearningLab = init;
})();
...
function switchTopTab(tabId, btn) {
  ...
  // Always sync S state before any tab renders
  ...
  if (tabId === 'stdsearch') {
    // Standards AI Search tab - nothing to init, config loaded from localStorage
  }
  if (tabId === 'testing') {
    setTimeout(() => { try {
      if(typeof propagate==='function') propagate(); // ensure S is fresh
      if(window._bt_active_cat==='data_analysis') renderDataAnalysis();
      else if(typeof renderBatteryTesting==='function') renderBatteryTesting(window._bt_active_cat);
    } catch(e){ console.warn('[testing init]',e); } }, 50);
  }
  if (tabId === 'dashboard') {
  ...
  if (tabId === 'standards') {
    setTimeout(() => { try { if(typeof propagate==='function') propagate(); } catch(e) {} }, 60);
  }
  if (tabId === 'approvals') {
    setTimeout(() => { try { if(typeof propagate==='function') propagate(); } catch(e) {} }, 60);
  }
...
function showSec(id, btn) {
  ...
  setTimeout(function() {
    ...
    try {
      ...
      if (id === 'dcir')       { try { if(typeof renderDCIRMap==='function') { propagate(); renderDCIRMap(); } } catch(e) {} }
      if (id === 'derating')   { try { if(typeof renderDerating==='function') { propagate(); renderDerating(); } } catch(e) {} }
      if (id === 'fuse')       { try { if(typeof renderFuseSelector==='function') { propagate(); renderFuseSelector(); } } catch(e) {} }
      if (id === 'sizing')     { try { if(typeof renderSizing==='function') { propagate(); renderSizing(); } } catch(e) {} }
      if (id === 'cellqual')   { try { if(typeof renderCellQual==='function') { propagate(); renderCellQual(); } } catch(e) {} }
      if (id === 'data_analysis'){ try { if(typeof renderDataAnalysis==='function') renderDataAnalysis(); } catch(e) {} }
    } catch(e) {
...
function pullEngToIntake() {
  const map = [
    ...
    ['t_ppeak',      'dvp-ppeak'],
    ['t_pcont',      'dvp-pcont'],
    ['t_imax_chg',   'dvp-imax_chg'],
    ['t_tr_warn',    'dvp-tr_warn'],
  ];
...
window.switchTopTab = function(tabId, btn) {
  ...
  // Hide engineering subnav on non-engineering tabs
  const _sn = document.getElementById('eng-subnav');
  if (_sn) _sn.style.display = tabId === 'engineering' ? 'flex' : 'none';
  if (tabId === 'engineering') {
...
const STDS_INGESTION_WORKFLOW = {"name": "BatteryMIS Standards Ingestion", "nodes": [{"id": "webhook-ingest", "name": "Webhook \u2014 Trigger Ingestion", "type": "n8n-nodes-base.webhook", "typeVersion": 1, "position": [240, 300], "parameters": {"path": "standards-ingest", "responseMode": "responseNode", "httpMethod": "POST"}}, {"id": "respond-start", "name": "Respond \u2014 Accepted", "type": "n8n-nodes-base.respondToWebhook", "typeVersion": 1, "position": [440, 200], "parameters": {"respondWith": "json", "responseBody": "={{ JSON.stringify({message: \"Ingestion started\", folder_id: $json.folder_id}) }}"}}, {"id": "gdrive-list", "name": "Google Drive \u2014 List PDFs", "type": "n8n-nodes-base.googleDrive", "typeVersion": 3, "position": [440, 380], "parameters": {"operation": "filesList", "folderId": "={{ $json.body.folder_id || \"YOUR_FOLDER_ID_HERE\" }}", "filter": {"mimeType": "application/pdf"}}, "credentials": {"googleDriveOAuth2Api": {"name": "Google Drive account"}}}, {"id": "split-files", "name": "Split \u2014 Each file", "type": "n8n-nodes-base.splitInBatches", "typeVersion": 3, "position": [640, 380], "parameters": {"batchSize": 1, "options": {}}}, {"id": "gdrive-download", "name": "Google Drive \u2014 Download PDF", "type": "n8n-nodes-base.googleDrive", "typeVersion": 3, "position": [840, 380], "parameters": {"operation": "download", "fileId": "={{ $json.id }}"}, "credentials": {"googleDriveOAuth2Api": {"name": "Google Drive account"}}}, {"id": "extract-text", "name": "Extract Text from PDF", "type": "n8n-nodes-base.extractFromFile", "typeVersion": 1, "position": [1040, 380], "parameters": {"operation": "pdf", "binaryPropertyName": "data"}}, {"id": "chunk-code", "name": "Code \u2014 Chunk text", "type": "n8n-nodes-base.code", "typeVersion": 2, "position": [1240, 380], "parameters": {"jsCode": "\nconst text = items[0].json.text || '';\nconst fileName = items[0].binary?.data?.fileName || 'unknown.pdf';\nconst CHUNK_SIZE = 512;\nconst OVERLAP = 64;\n\n// Split into sentences first, then chunk\nconst sentences = text.replace(/\\n+/g, ' ').split(/(?<=[.!?])\\s+/);\nconst chunks = [];\nlet current = '';\nlet wordCount = 0;\n\nfor (const sentence of sentences) {\n  const words = sentence.split(' ');\n  if (wordCount + words.length > CHUNK_SIZE && current.length > 0) {\n    chunks.push(current.trim());\n    // Overlap: keep last OVERLAP words\n    const overlapWords = current.split(' ').slice(-OVERLAP);\n    current = overlapWords.join(' ') + ' ' + sentence + ' ';\n    wordCount = overlapWords.length + words.length;\n  } else {\n    current += sentence + ' ';\n    wordCount += words.length;\n  }\n}\nif (current.trim()) chunks.push(current.trim());\n\n// Build output items with metadata\nreturn chunks.map((chunk, i) => ({\n  json: {\n    chunk_text: chunk,\n    chunk_index: i,\n    total_chunks: chunks.length,\n    source_file: fileName,\n    standard_name: fileName.replace('.pdf',''),\n    metadata: {\n      source: fileName,\n      chunk: i,\n      total: chunks.length\n    }\n  }\n}));\n"}}, {"id": "embed-ollama", "name": "HTTP \u2014 Ollama Embed", "type": "n8n-nodes-base.httpRequest", "typeVersion": 4, "position": [1440, 380], "parameters": {"method": "POST", "url": "http://localhost:11434/api/embeddings", "sendBody": true, "specifyBody": "json", "jsonBody": "={{ JSON.stringify({ model: \"nomic-embed-text\", prompt: $json.chunk_text }) }}", "options": {"timeout": 60000}}}, {"id": "store-chroma", "name": "HTTP \u2014 Store in Chroma", "type": "n8n-nodes-base.httpRequest", "typeVersion": 4, "position": [1640, 380], "parameters": {"method": "POST", "url": "http://localhost:8000/api/v1/collections/battery_standards/upsert", "sendBody": true, "specifyBody": "json", "jsonBody": "={{ JSON.stringify({\n  ids: [ $('Code \u2014 Chunk text').item.json.source_file.replace('.pdf','') + '_' + $('Code \u2014 Chunk text').item.json.chunk_index ],\n  embeddings: [ $json.embedding ],\n  documents: [ $('Code \u2014 Chunk text').item.json.chunk_text ],\n  metadatas: [ $('Code \u2014 Chunk text').item.json.metadata ]\n}) }}", "options": {"timeout": 30000}}}], "connections": {"Webhook \u2014 Trigger Ingestion": {"main": [[{"node": "Respond \u2014 Accepted", "type": "main", "index": 0}, {"node": "Google Drive \u2014 List PDFs", "type": "main", "index": 0}]]}, "Google Drive \u2014 List PDFs": {"main": [[{"node": "Split \u2014 Each file", "type": "main", "index": 0}]]}, "Split \u2014 Each file": {"main": [[{"node": "Google Drive \u2014 Download PDF", "type": "main", "index": 0}]]}, "Google Drive \u2014 Download PDF": {"main": [[{"node": "Extract Text from PDF", "type": "main", "index": 0}]]}, "Extract Text from PDF": {"main": [[{"node": "Code \u2014 Chunk text", "type": "main", "index": 0}]]}, "Code \u2014 Chunk text": {"main": [[{"node": "HTTP \u2014 Ollama Embed", "type": "main", "index": 0}]]}, "HTTP \u2014 Ollama Embed": {"main": [[{"node": "HTTP \u2014 Store in Chroma", "type": "main", "index": 0}]]}}, "settings": {"executionOrder": "v1"}, "staticData": null, "tags": [{"name": "BatteryMIS"}, {"name": "Standards"}]};
const STDS_QUERY_WORKFLOW = {"name": "BatteryMIS Standards Query", "nodes": [{"id": "webhook-query", "name": "Webhook \u2014 Query", "type": "n8n-nodes-base.webhook", "typeVersion": 1, "position": [240, 300], "parameters": {"path": "standards-query", "responseMode": "responseNode", "httpMethod": "POST"}}, {"id": "test-check", "name": "IF \u2014 Test ping", "type": "n8n-nodes-base.if", "typeVersion": 2, "position": [440, 300], "parameters": {"conditions": {"boolean": [{"value1": "={{ !!$json.body.test }}", "value2": true}]}}}, {"id": "respond-test", "name": "Respond \u2014 Test OK", "type": "n8n-nodes-base.respondToWebhook", "typeVersion": 1, "position": [640, 200], "parameters": {"respondWith": "json", "responseBody": "={ \"status\": \"ok\", \"service\": \"BatteryMIS Standards Query\" }"}}, {"id": "embed-question", "name": "HTTP \u2014 Embed question", "type": "n8n-nodes-base.httpRequest", "typeVersion": 4, "position": [640, 380], "parameters": {"method": "POST", "url": "http://localhost:11434/api/embeddings", "sendBody": true, "specifyBody": "json", "jsonBody": "={{ JSON.stringify({ model: \"nomic-embed-text\", prompt: $json.body.question }) }}", "options": {"timeout": 30000}}}, {"id": "search-chroma", "name": "HTTP \u2014 Search Chroma", "type": "n8n-nodes-base.httpRequest", "typeVersion": 4, "position": [840, 380], "parameters": {"method": "POST", "url": "http://localhost:8000/api/v1/collections/battery_standards/query", "sendBody": true, "specifyBody": "json", "jsonBody": "={{ JSON.stringify({ query_embeddings: [$json.embedding], n_results: 8, include: [\"documents\",\"metadatas\",\"distances\"] }) }}", "options": {"timeout": 30000}}}, {"id": "build-context", "name": "Code \u2014 Build RAG context", "type": "n8n-nodes-base.code", "typeVersion": 2, "position": [1040, 380], "parameters": {"jsCode": "\nconst chromaResult = items[0].json;\nconst originalQuery = $('Webhook \u2014 Query').item.json.body;\n\nconst docs = chromaResult.documents?.[0] || [];\nconst metas = chromaResult.metadatas?.[0] || [];\nconst dists = chromaResult.distances?.[0] || [];\n\nconst SIMILARITY_THRESHOLD = 1.0; // Chroma L2 distance \u2014 lower = more similar\nconst relevant = docs.filter((_, i) => dists[i] < SIMILARITY_THRESHOLD);\n\nif (relevant.length === 0) {\n  return [{ json: { found: false, question: originalQuery.question, context: '', sources: [] } }];\n}\n\n// Build context string with source attribution\nlet context = '';\nconst sources = [];\nconst seen = new Set();\n\ndocs.forEach((doc, i) => {\n  if (dists[i] >= SIMILARITY_THRESHOLD) return;\n  const meta = metas[i] || {};\n  const srcName = meta.source?.replace('.pdf','') || 'Unknown';\n  context += `--- Source: ${srcName} ---\\n${doc}\\n\\n`;\n  if (!seen.has(srcName)) {\n    seen.add(srcName);\n    sources.push({ name: srcName, relevance: +(1 - dists[i]/2).toFixed(3) });\n  }\n});\n\n// Apply filters from query if provided\nconst filters = originalQuery.filters || {};\nconst filterStr = Object.entries(filters).filter(([,v])=>v).map(([k,v])=>`${k}: ${v}`).join(', ');\n\nreturn [{ json: {\n  found: true,\n  question: originalQuery.question,\n  filters: filterStr,\n  context: context.substring(0, 6000), // Limit context for LLM\n  sources: sources.slice(0,6)\n} }];\n"}}, {"id": "check-found", "name": "IF \u2014 Standards found", "type": "n8n-nodes-base.if", "typeVersion": 2, "position": [1240, 380], "parameters": {"conditions": {"boolean": [{"value1": "={{ $json.found }}", "value2": true}]}}}, {"id": "respond-not-found", "name": "Respond \u2014 Not found", "type": "n8n-nodes-base.respondToWebhook", "typeVersion": 1, "position": [1440, 480], "parameters": {"respondWith": "json", "responseBody": "={{ JSON.stringify({ found: false, answer: null, sources: [], question: $json.question }) }}"}}, {"id": "build-prompt", "name": "Code \u2014 Build LLM prompt", "type": "n8n-nodes-base.code", "typeVersion": 2, "position": [1440, 300], "parameters": {"jsCode": "\nconst data = items[0].json;\nconst filterNote = data.filters ? `\\nFilters applied: ${data.filters}` : '';\n\nconst systemPrompt = `You are a battery engineering standards expert assistant. \nYour role is to answer questions about EV battery test procedures using ONLY the provided standard excerpts.\n\nRules:\n- Answer ONLY from the provided context. Do not invent information.\n- Always cite the source standard by name.\n- If the context partially answers the question, say what is covered and what is missing.\n- Be precise about test conditions: temperatures, voltages, durations, pass/fail criteria.\n- Use bullet points for test steps and conditions.\n- If the context does not answer the question at all, respond with \"STANDARD_NOT_FOUND\".`;\n\nconst userPrompt = `Question: ${data.question}${filterNote}\n\nRelevant standard excerpts:\n${data.context}\n\nProvide a comprehensive answer citing specific clauses and conditions from the standards above.`;\n\nreturn [{ json: { ...data, system_prompt: systemPrompt, user_prompt: userPrompt } }];\n"}}, {"id": "ollama-llm", "name": "HTTP \u2014 Ollama LLM", "type": "n8n-nodes-base.httpRequest", "typeVersion": 4, "position": [1640, 300], "parameters": {"method": "POST", "url": "http://localhost:11434/api/generate", "sendBody": true, "specifyBody": "json", "jsonBody": "={{ JSON.stringify({\n  model: \"llama3\",\n  prompt: $json.user_prompt,\n  system: $json.system_prompt,\n  stream: false,\n  options: { temperature: 0.1, num_ctx: 8192 }\n}) }}", "options": {"timeout": 120000}}}, {"id": "format-response", "name": "Code \u2014 Format final answer", "type": "n8n-nodes-base.code", "typeVersion": 2, "position": [1840, 300], "parameters": {"jsCode": "\nconst llmResponse = items[0].json.response || '';\nconst buildContext = $('Code \u2014 Build RAG context').item.json;\n\nconst notFound = llmResponse.includes('STANDARD_NOT_FOUND');\n\nreturn [{ json: {\n  found: !notFound,\n  answer: notFound ? null : llmResponse.trim(),\n  sources: buildContext.sources || [],\n  question: buildContext.question,\n  model: 'llama3 (Ollama local)'\n} }];\n"}}, {"id": "respond-answer", "name": "Respond \u2014 Answer", "type": "n8n-nodes-base.respondToWebhook", "typeVersion": 1, "position": [2040, 300], "parameters": {"respondWith": "json", "responseBody": "={{ JSON.stringify($json) }}"}}], "connections": {"Webhook \u2014 Query": {"main": [[{"node": "IF \u2014 Test ping", "type": "main", "index": 0}]]}, "IF \u2014 Test ping": {"main": [[{"node": "Respond \u2014 Test OK", "type": "main", "index": 0}], [{"node": "HTTP \u2014 Embed question", "type": "main", "index": 0}]]}, "HTTP \u2014 Embed question": {"main": [[{"node": "HTTP \u2014 Search Chroma", "type": "main", "index": 0}]]}, "HTTP \u2014 Search Chroma": {"main": [[{"node": "Code \u2014 Build RAG context", "type": "main", "index": 0}]]}, "Code \u2014 Build RAG context": {"main": [[{"node": "IF \u2014 Standards found", "type": "main", "index": 0}]]}, "IF \u2014 Standards found": {"main": [[{"node": "Code \u2014 Build LLM prompt", "type": "main", "index": 0}], [{"node": "Respond \u2014 Not found", "type": "main", "index": 0}]]}, "Code \u2014 Build LLM prompt": {"main": [[{"node": "HTTP \u2014 Ollama LLM", "type": "main", "index": 0}]]}, "HTTP \u2014 Ollama LLM": {"main": [[{"node": "Code \u2014 Format final answer", "type": "main", "index": 0}]]}, "Code \u2014 Format final answer": {"main": [[{"node": "Respond \u2014 Answer", "type": "main", "index": 0}]]}}, "settings": {"executionOrder": "v1"}, "tags": [{"name": "BatteryMIS"}, {"name": "Standards"}]};

/* ═══════════════════════════════════════════════
   STANDARDS AI SEARCH  - BatteryMIS v2.0
   Local RAG: n8n → Chroma → Ollama → Answer
   ═══════════════════════════════════════════════ */
(function(){
  const STORE_KEY = 'battmis_stds_config_v1';
  let _history = [];
  ...
  window.stdsShowSetup = function(){
    const el = document.getElementById('stds_setup');
    if(!el) return;
    const visible = el.style.display !== 'none';
    el.style.display = visible ? 'none' : 'block';
    if(!visible){
      const c = cfg();
      const set = id => { const el=document.getElementById(id); if(el&&c[id]) el.value=c[id]; };
      set('stds_n8n_url'); set('stds_ingest_url'); set('stds_folder_id');
    }
  };

  window.stdsSaveConfig = function(){
    const get = id => (document.getElementById(id)||{}).value||'';
    const config = {
      stds_n8n_url: get('stds_n8n_url'),
      stds_ingest_url: get('stds_ingest_url'),
      stds_folder_id: get('stds_folder_id'),
    };
    localStorage.setItem(STORE_KEY, JSON.stringify(config));
    stdsUpdateStatus('configured');
    document.getElementById('stds_setup_msg').textContent = '✓ Saved. Click Test connection to verify n8n is reachable.';
    document.getElementById('stds_setup_msg').style.color = 'var(--teal)';
  };

  window.stdsTestConnection = function(){
    const url = cfg().stds_n8n_url;
    if(!url){ alert('Enter your n8n webhook URL first.'); return; }
    const msg = document.getElementById('stds_setup_msg');
    msg.textContent = 'Testing…'; msg.style.color = 'var(--text3)';
    fetch(url, { method:'POST', headers:{'Content-Type':'application/json'},
      body: JSON.stringify({test: true}) })
      .then(r => {
        if(r.ok){ msg.textContent = '✓ n8n reachable'; msg.style.color='var(--teal)'; stdsUpdateStatus('online'); }
        else { msg.textContent = '⚠ n8n returned '+r.status; msg.style.color='var(--y)'; }
      })
      .catch(e => { msg.textContent = '✗ Cannot reach n8n: '+e.message; msg.style.color='var(--r)'; stdsUpdateStatus('offline'); });
  };

  window.stdsTriggerIngest = function(){
    const c = cfg();
    if(!c.stds_ingest_url){ alert('Enter the ingestion webhook URL in Setup first.'); return; }
    const msg = document.getElementById('stds_setup_msg');
    msg.textContent = '⏳ Triggering ingestion… this may take several minutes for 150 PDFs.';
    msg.style.color = 'var(--y)';
    fetch(c.stds_ingest_url, { method:'POST', headers:{'Content-Type':'application/json'},
      body: JSON.stringify({ folder_id: c.stds_folder_id, action:'ingest' }) })
      .then(r => r.json())
      .then(d => {
        msg.textContent = '✓ Ingestion started: ' + (d.message||JSON.stringify(d));
        msg.style.color = 'var(--teal)';
      })
      .catch(e => { msg.textContent = '✗ Error: '+e.message; msg.style.color='var(--r)'; });
  };

  function stdsUpdateStatus(state){
    const dot = document.getElementById('stds_status_dot');
    const txt = document.getElementById('stds_status_txt');
    if(!dot||!txt) return;
    const map = {
      configured: ['#f5c518','Configured - not tested'],
      online:     ['#00d4aa','Online'],
      offline:    ['#ff4d6d','Offline'],
      thinking:   ['#4a9eff','Searching…'],
    };
    const [color, label] = map[state]||['#3a567a','Not configured'];
    dot.style.background = color;
    txt.textContent = label;
  }

  window.stdsQuick = function(question){
    document.getElementById('stds_q_text').value = question;
    stdsAsk();
  };

  window.stdsAsk = async function(){
    const question = (document.getElementById('stds_q_text')||{}).value?.trim();
    if(!question){ return; }
    const url = cfg().stds_n8n_url;
    if(!url){
      stdsAddBubble('Please configure the n8n webhook URL first. Click ⚙ Setup.', 'not-found');
      return;
    }

    const qType    = (document.getElementById('stds_q_type')||{}).value||'';
    const qFamily  = (document.getElementById('stds_q_family')||{}).value||'';
    const qVtype   = (document.getElementById('stds_q_vtype')||{}).value||'';
    const qRegion  = (document.getElementById('stds_q_region')||{}).value||'';

    // Build structured query object
    const payload = { question, filters:{ test_type:qType, standard_family:qFamily, vehicle_type:qVtype, region:qRegion } };

    // Hide empty state
    const empty = document.getElementById('stds_empty');
    if(empty) empty.style.display = 'none';

    stdsAddBubble(question + (qType||qFamily||qVtype||qRegion ?
      '<br><span style="font-size:10px;opacity:.6">' +
      [qType,qFamily,qVtype,qRegion].filter(Boolean).join(' · ') + '</span>' : ''), 'q');

    const loadId = 'stds_load_' + Date.now();
    stdsAddBubble('<span id="'+loadId+'"><span style="color:var(--teal)">⏳</span> Searching your standards…</span>', 'a', loadId);
    stdsUpdateStatus('thinking');
    ...
    try {
      ...
      // { answer: "...", sources: [{name:"IEC 62619", clause:"4.3.2", relevance:0.92},...], found: true }
      if(data.found === false || !data.answer){
        if(el) el.parentElement.className = 'stds-not-found';
        if(el) el.innerHTML = '📭 Standard not found for this query.<br><span style="font-size:11px;opacity:.8">No matching clauses were retrieved from your 150 standards. Try broadening the filters or rephrasing the question.</span>';
      } else {
        let html = '<div style="font-size:12px;line-height:1.75;color:var(--text)">' + (data.answer||'').replace(/\n/g,'<br>') + '</div>';
        if(data.sources && data.sources.length){
          html += '<div style="margin-top:10px;font-size:10px;color:var(--text3)">Sources:</div>';
          html += '<div style="display:flex;flex-wrap:wrap;gap:4px;margin-top:4px">';
          data.sources.forEach(s => {
            html += '<span class="stds-source-tag">📄 ' + (s.name||'') + (s.clause?' §'+s.clause:'') + '</span>';
          });
          html += '</div>';
        }
      ...
      const el = document.getElementById(loadId);
      if(el) el.innerHTML = '<span style="color:var(--r)">✗ Error: '+e.message+'</span><br><span style="font-size:11px;color:var(--text3)">Check that n8n is running and the webhook URL is correct.</span>';
    }
    stdsUpdateStatus('online');
    _history.push({ q: question, payload });
    document.getElementById('stds_q_text').value = '';
  };

  function stdsAddBubble(html, type, id){
    const hist = document.getElementById('stds_history');
    if(!hist) return;
    const div = document.createElement('div');
    div.className = type==='q' ? 'stds-q-bubble' : type==='not-found' ? 'stds-not-found' : 'stds-a-bubble';
    div.innerHTML = html;
    if(id) div.id = id;
  ...
  /* ── n8n Workflow JSON downloads ── */
  window.stdsDownloadIngestion = function(){
    const wf = JSON.stringify(STDS_INGESTION_WORKFLOW, null, 2);
    const a = document.createElement('a');
    a.href = 'data:application/json,' + encodeURIComponent(wf);
    a.download = 'BatteryMIS_Standards_Ingestion_Workflow.json';
    a.click();
  };
  window.stdsDownloadQuery = function(){
    const wf = JSON.stringify(STDS_QUERY_WORKFLOW, null, 2);
    const a = document.createElement('a');
    a.href = 'data:application/json,' + encodeURIComponent(wf);
    a.download = 'BatteryMIS_Standards_Query_Workflow.json';
    a.click();
  };

  // Load saved config on init
  const saved = cfg();
  if(saved.stds_n8n_url) stdsUpdateStatus('configured');
})();
...
// ═══════════════════════════════════════════════════════
// PERSIST_KEY: always use window.PERSIST_KEY if set (per-user login key)
// Falls back to 'battmis_v1' for guest/unauthenticated sessions
const PERSIST_KEY = 'battmis_v1'; // initial default
function _pk() { return window.PERSIST_KEY || PERSIST_KEY; }
...
const SOURCE_FIELD_IDS = [
  ...
  // New Project Targets (added in redesign)
  't_dod','t_tpeak','t_cpeak','t_varch','t_imax_chg',
  't_ved','t_pmass_max','t_pvol_max','t_tr_warn',
  't_cont_life','t_soc_acc','t_soh_acc','t_warranty','t_warranty_cycles',
  't_cell_cost','t_tstg_hi','t_tstg_lo','t_ship_soc',
  't_vib_req','t_salt_req','t_shock_req','t_drop_req',
  't_cert_primary','t_cert_emc','t_cert_un383','t_cert_other',
  // Extended Check Targets (editable in Project Targets → Extended Check Targets card)
  't_ir_bol_max','t_ir_growth_max','t_vdrop_cold_max','t_bms_vwindow_min',
  't_pc_time_max','t_hv_class','t_tchg_window_min','t_regen_soc_margin',
  't_trp','t_fire','t_hipot','t_bms_own','t_can_speed','t_comm_proto',
  't_fusa','t_fusa_custom','t_dod',
  // Req Mapping fields
  'req_chem','req_s_range','req_ppeak10','req_heat','req_liqcool',
  'req_maxflow','req_dp','req_rh',
  // Cell Inputs extras
  'c_supplier','c_model','c_cps_note',
  // Business & Cost - BOM component costs (saved so they don't reset)
  'gap_cell_cost','gap_bms_cost','gap_housing_cost','gap_tms_cost','gap_pdu_cost',
  'gap_fuel_cost','gap_fuel_cons','gap_elec_cost',
  'bom_assembly','bom_testing','bom_tooling','bom_logistics',
  'bom_overhead_pct','bom_profit_pct',
  // Drive cycle manual
...
function saveProject() {
  try {
    ...
    // Save to BOTH keys: session key AND fallback key
    // This ensures data survives across login key changes
    localStorage.setItem(_pk(), json);
    localStorage.setItem('battmis_v1', json); // always keep fallback copy
  } catch(e) {}
}

function loadProject() {
  try {
    // Try session key first, then fallback to 'battmis_v1'
    let raw = localStorage.getItem(_pk());
    if (!raw) raw = localStorage.getItem('battmis_v1');
    if (!raw) return false;
    const data = JSON.parse(raw);
...
function markDerivedFields() {
  // NEVER lock Project Target or Cell Input source fields
  const SOURCE_NEVER_LOCK = new Set(SOURCE_FIELD_IDS);
  DERIVED_FIELD_IDS.forEach(id => {
    if (SOURCE_NEVER_LOCK.has(id)) return; // skip source fields
    const el = document.getElementById(id);
    if (!el) return;
...
// ═══════════════════════════════════════════════
// COMMON STATE: window.BMS aliases window.S
// All modules can use BMS.c_vnom, BMS.t_ppeak etc.
// ═══════════════════════════════════════════════
window.BMS = window.S; // live alias - same object

// Auto-propagate + re-render on any source field input
document.addEventListener('input', function(e) {
  if (!e.target.id || !SOURCE_FIELD_IDS.includes(e.target.id)) return;
  clearTimeout(window._srcDebounce);
  window._srcDebounce = setTimeout(() => {
    try { if (typeof propagate === 'function') propagate(); } catch(err) {}
    // Re-render active module if it's a "linked" module
    const activeSec = document.querySelector('.sec.active');
    if (!activeSec) return;
    const sid = activeSec.id;
    const re = {
      sizing:     () => typeof renderSizing==='function'&&renderSizing(),
    ...
    if (re[sid]) { try { re[sid](); } catch(err) {} }
    // Always update compliance panels
    ...
    // Also re-render BT if active top tab
    const btPanel = document.getElementById('panel-testing');
...
// Auto-save on ANY input change (belt-and-suspenders)
// Also save BOM cost fields immediately
document.addEventListener('change', function(e) {
  if (e.target.dataset.persist === '1') {
    try { saveProject(); } catch(err) {}
  }
});
document.addEventListener('change', function(e) {
  if(e.target.id && SOURCE_FIELD_IDS.includes(e.target.id)) {
    try { saveProject(); } catch(err) {}
  }
});
...
const S = {
  ...
  // Energy & Capacity
  t_emin:40, t_emax:60, t_eu_min:35, t_auto:4, t_dod:1.0,
  t_ged:100, t_ved:200, t_pmass_max:500, t_pvol_max:300,
  t_vmin_sys:250, t_vmax_sys:500, t_varch:400,

  // Electrical Performance
  t_ppeak:80, t_pcont:50, t_tpeak:30, t_cpeak:2.0, t_ppeak10:65,
  t_pdc:60,   t_pac:22,   t_cchg:0.5, t_imax_chg:120,

  // Thermal
  t_top_lo:-20, t_top_hi:45, t_tcell_max:55, t_tgrad:5,
  t_tchg_lo:-15, t_tchg_hi:45, t_cooling:'liquid',
  t_tstg_hi:65, t_tstg_lo:-25, t_ship_soc:30,

  // Environmental & Safety
  t_ip:'IP67', t_alt:3000,
  t_iso_r:'500', t_hipot:'2xVmax+1000V', t_trp:'required', t_tr_warn:5,
  t_cert_primary:'', t_cert_un383:'', t_cert_emc:'', t_cert_other:'',
  // Extended Check Targets
  t_ir_bol_max:500, t_ir_growth_max:2.0, t_vdrop_cold_max:5,
  t_bms_vwindow_min:1.0, t_pc_time_max:5, t_hv_class:'B',
  t_tchg_window_min:40, t_regen_soc_margin:10,
  t_vib_req:'', t_salt_req:'', t_shock_req:'', t_drop_req:'',

  // Lifetime & Reliability
  t_cycles:3000, t_soh_eol:80, t_years:10,
  t_soh5:90, t_soh10:80, t_days_yr:200, t_cycles_day:1.0,
  t_cont_life:10000, t_soc_acc:3, t_soh_acc:5,
  t_bms_own:'supplier', t_can_speed:'500k', t_comm_proto:'CAN',

  // Business & Cost
  t_cost:130, t_cell_cost:80, t_warranty:5, t_warranty_cycles:1500,
  ...
  // Derived (recalculated on every propagate)
  S_total:112, V_nom_pack:358.4, V_max_pack:408.8, V_min_pack:224,
  V_eff_min:0, V_eff_max:0, V_eff_min_cell:0, V_eff_max_cell:0, SoC_lo:0, SoC_hi:100,
  V_dod_lo_cell:0, V_dod_hi_cell:0, V_dod_lo_pack:0, V_dod_hi_pack:0,
  V_op_min_pack:0, V_op_max_pack:0, soc_lo:0, soc_hi:100,
  E_gross:43.0, E_usable:38.0, Q_pack:120,
  pack_mass:0, _packIR_bol:0,

  // Drive Cycle (set by drive-cycle-fix.js)
  dc_pavg:0, dc_ppeak:0, dc_dur_h:0,
};
...
function propagate() {
  ...
  S.t_ip        = document.getElementById('t_ip')?.value || S.t_ip || 'IP67';
  S.t_alt       = g('t_alt')       ?? S.t_alt;
  S.t_ved       = g('t_ved')       ?? S.t_ved;
  S.t_pmass_max = g('t_pmass_max') ?? S.t_pmass_max;
  S.t_pvol_max  = g('t_pvol_max')  ?? S.t_pvol_max;
  S.t_tr_warn   = g('t_tr_warn')   ?? (S.t_tr_warn || 5);
  S.t_iso_r     = document.getElementById('t_iso_r')?.value || S.t_iso_r || '500';
  S.t_bms_own   = document.getElementById('t_bms_own')?.value || S.t_bms_own || 'supplier';
  S.t_warranty  = g('t_warranty')  ?? (S.t_warranty || 5);
  S.t_tstg_hi   = g('t_tstg_hi')   ?? (S.t_tstg_hi || 65);
  S.t_tstg_lo   = g('t_tstg_lo')   ?? (S.t_tstg_lo || -25);
  S.t_ship_soc  = g('t_ship_soc')  ?? (S.t_ship_soc || 30);
  S.t_cell_cost     = g('t_cell_cost')     ?? S.t_cell_cost;
  S.t_warranty_cycles=g('t_warranty_cycles')?? (S.t_warranty_cycles||1500);
  S.t_cont_life     = g('t_cont_life')     ?? (S.t_cont_life||10000);
  S.t_soc_acc       = g('t_soc_acc')       ?? (S.t_soc_acc||3);
  S.t_soh_acc       = g('t_soh_acc')       ?? (S.t_soh_acc||5);
  S.t_tpeak         = g('t_tpeak')         ?? (S.t_tpeak||30);
  S.t_cpeak         = g('t_cpeak')         ?? (S.t_cpeak||2.0);
  S.t_varch         = g('t_varch')         ?? (S.t_varch||400);
  S.t_trp           = document.getElementById('t_trp')?.value   || S.t_trp  || 'required';
  S.t_fire          = document.getElementById('t_fire')?.value  || S.t_fire || 'notreq';
  S.t_can_speed     = document.getElementById('t_can_speed')?.value || S.t_can_speed || '500k';
  S.t_comm_proto    = document.getElementById('t_comm_proto')?.value|| S.t_comm_proto|| 'CAN';
  S.t_cooling       = document.getElementById('t_cooling')?.value   || S.t_cooling   || 'liquid';
  // Certification (open text - blank = not specified = pass)
  S.t_cert_primary  = document.getElementById('t_cert_primary')?.value || S.t_cert_primary || '';
  // Extended Check Targets
  ['t_ir_bol_max','t_ir_growth_max','t_vdrop_cold_max','t_bms_vwindow_min',
   't_pc_time_max','t_tchg_window_min','t_regen_soc_margin'].forEach(id => {
    const el = document.getElementById(id);
    if (el && el.value !== '') S[id] = +el.value;
  });
  const _hv_el = document.getElementById('t_hv_class');
  if (_hv_el) S.t_hv_class = _hv_el.value;
  S.t_cert_un383    = document.getElementById('t_cert_un383')?.value   || S.t_cert_un383   || '';
  S.t_cert_emc      = document.getElementById('t_cert_emc')?.value     || S.t_cert_emc     || '';
  S.t_cert_other    = document.getElementById('t_cert_other')?.value   || S.t_cert_other   || '';
  // New fields from redesigned Project Targets
  S.t_tpeak         = +(document.getElementById('t_tpeak')?.value)      || S.t_tpeak     || 30;
  S.t_cpeak         = +(document.getElementById('t_cpeak')?.value)      || S.t_cpeak     || 2.0;
  S.t_dod           = +(document.getElementById('t_dod')?.value)        || S.t_dod       || 1.0;
  S.t_imax_chg      = +(document.getElementById('t_imax_chg')?.value)   || S.t_imax_chg  || 120;
  S.t_ved           = +(document.getElementById('t_ved')?.value)        || S.t_ved       || 200;
  S.t_pmass_max     = +(document.getElementById('t_pmass_max')?.value)  || S.t_pmass_max || 500;
  S.t_pvol_max      = +(document.getElementById('t_pvol_max')?.value)   || S.t_pvol_max  || 300;
  // Environmental test requirements (open text - blank = pass)
  S.t_vib_req  = document.getElementById('t_vib_req')?.value   || S.t_vib_req   || '';
  S.t_salt_req = document.getElementById('t_salt_req')?.value  || S.t_salt_req  || '';
  S.t_shock_req= document.getElementById('t_shock_req')?.value || S.t_shock_req || '';
  S.t_drop_req = document.getElementById('t_drop_req')?.value  || S.t_drop_req  || '';
  S.markets    = g('t_markets') || S.markets;

  // ── Read Cell Inputs ──
  S.c_supplier    = document.getElementById('c_supplier')?.value || S.c_supplier || '';
  S.c_model       = document.getElementById('c_model')?.value      || S.c_model      || '';
  ...
  // ── Derive pack values ──
  // ALWAYS recompute S_total from DOM - never use stale S.S_total
  const _cps_dom = +(document.getElementById('c_cps')?.value||0);
  const _css_dom = +(document.getElementById('c_ss')?.value||0);
  const _cpp_dom = +(document.getElementById('c_pp')?.value||0);
  if (_cps_dom > 0) S.c_cps = _cps_dom;
  if (_css_dom > 0) S.c_ss  = _css_dom;
  if (_cpp_dom > 0) S.c_pp  = _cpp_dom;
  S.S_total     = S.c_cps * S.c_ss;
  // Config label: the actual derived result from Cell Inputs
  S.config_label = `${S.c_cps}×${S.c_ss}=${S.S_total}S / ${S.c_pp}P`;
  S.config_short = `${S.S_total}S/${S.c_pp}P`;
  // config_cell format matches Derived Pack Specifications: "c_cps×c_ss-c_ah"
  S.config_cell  = `${S.c_cps}×${S.c_ss}-${S.c_ah}`;
  // Timestamp of last propagate (for debug)
  ...
  if(!S.V_eff_min||S.V_eff_min===0){S.V_eff_min=S.V_min_pack;S.V_eff_max=S.V_max_pack;S.V_eff_min_cell=S.c_vmin;S.V_eff_max_cell=S.c_vmax;}
  S.E_gross     = (S.c_vnom * S.c_ah * S.S_total * S.c_pp) / 1000;
  S.pack_mass   = (S.c_cps * S.c_ss * S.c_pp * S.c_mass / 1000) + S.c_housing_mass;
  S.E_usable   = S.E_gross * (S.t_dod || 1.0); // refreshed by calcEnergy later
  ...
  try {
    ...
    if (ocvPts.length >= 2) {
      ...
      if (vLoCell > 0 && vHiCell > 0) {
        // ── Store DoD voltage bounds in S for use across all modules ──
        S.V_dod_lo_cell = vLoCell;   // V/cell at SoC_lo (DoD lower bound)
        S.V_dod_hi_cell = vHiCell;   // V/cell at SoC_hi (DoD upper bound)
        S.V_dod_lo_pack = vLoPack;   // Pack V at SoC_lo
        S.V_dod_hi_pack = vHiPack;   // Pack V at SoC_hi
        S.soc_lo        = socLo;
        S.soc_hi        = socHi;
        // V_eff aliases (used by Battery Testing, TDA, Cell Qual)
        S.V_eff_min_cell = vLoCell;  S.V_eff_max_cell = vHiCell;
        S.V_eff_min      = vLoPack;  S.V_eff_max      = vHiPack;
        S.SoC_lo         = socLo;    S.SoC_hi         = socHi;
        // FIX: Auto-update c_vmax / c_vmin from OCV table at 25°C
        if (vHiCell > 0 && vLoCell > 0) {
          const vmaxEl = document.getElementById('c_vmax');
          const vminEl = document.getElementById('c_vmin');
          if (vmaxEl && vmaxEl.dataset.ocvLinked !== 'manual') {
            vmaxEl.value = vHiCell.toFixed(3);
            vmaxEl.dataset.ocvLinked = 'auto';
            S.c_vmax = vHiCell;
          }
          if (vminEl && vminEl.dataset.ocvLinked !== 'manual') {
            vminEl.value = vLoCell.toFixed(3);
            vminEl.dataset.ocvLinked = 'auto';
            S.c_vmin = vLoCell;
          }
        }
        // Update operative V_min/V_max to DoD-based values (these override cell absolute limits)
        S.V_op_min_pack = vLoPack;   // usable minimum pack voltage
        S.V_op_max_pack = vHiPack;   // usable maximum pack voltage
        // Reflect back to system voltage fields if not manually overridden
        // t_vmin_sys / t_vmax_sys are user Project Targets - never auto-overwritten by OCV
        items.push(
...
function calcCurrent() {
  ...
  // Temps span Project Targets T_op range + cell max
  const _tlo = Math.min(-20, S.t_top_lo ?? -20);
  const _thi = Math.max(55,  S.t_tcell_max ?? 55);
  temps = [];
  for (let _T = Math.floor(_tlo/10)*10; _T <= _thi; _T += 5) temps.push(_T);

  // P_ref: use calculated P_electric; if zero, fall back to Project Target P_cont
  // This ensures the matrix shows meaningful data even when sub-loads aren't filled
  const P_ref = P_electric > 0 ? P_electric : (S.t_pcont || 50);
  const I_ref  = P_ref * 1000 / Vnom;
  const P_peak_ref = S.t_ppeak || (P_ref * 1.5);

  let mt = '<table style="width:100%"><tr>'
    + '<th>Ambient (°C)</th><th>Temp Factor</th>'
    + '<th>Power (kW)</th><th>Current (A)</th><th>C-rate</th>'
    + '<th>vs P_cont</th><th>Op. Range</th></tr>';
  temps.forEach(T => {
    const Tf   = T < 0 ? (1 + Math.abs(T)*0.015) : T > 35 ? (1 + (T-35)*0.012) : 1.0;
    const Pt   = P_ref * Tf / T_fac;
    const It   = Pt * 1000 / Vnom;
    const Ct   = Qah > 0 ? It / Qah : 0;
    const inOp = T >= (S.t_top_lo ?? -20) && T <= (S.t_top_hi ?? 55);
    const vsPcont = S.t_pcont > 0 ? (Pt / S.t_pcont * 100).toFixed(0) + '%' : '-';
    const pContOk = S.t_pcont > 0 ? Pt <= S.t_pcont * 1.05 : true;
    const isRef  = Math.abs(T - Tamb) < 3; // highlight current ambient
    mt += '<tr style="' + (isRef ? 'background:rgba(0,212,170,.08);' : T===25 ? 'background:rgba(74,158,255,.05);' : '') + '">'
      + '<td style="font-weight:' + (isRef?'700':'400') + ';color:' + (!inOp?'var(--warn)':'inherit') + '">'
        + T + '°C' + (isRef?' ←now':'') + (T===25?' ★':'') + '</td>'
      + '<td style="font-family:var(--mono);color:' + (Tf>1.15?'var(--warn)':Tf<1?'var(--b)':'inherit') + '">' + Tf.toFixed(3) + '×</td>'
      + '<td style="font-family:var(--mono)">' + Pt.toFixed(1) + '</td>'
      + '<td style="font-family:var(--mono)">' + It.toFixed(0) + '</td>'
      + '<td style="font-family:var(--mono);color:' + (Ct>2?'var(--r)':Ct>1?'var(--warn)':'inherit') + '">' + Ct.toFixed(3) + '</td>'
      + '<td style="font-family:var(--mono);color:' + (pContOk?'var(--g)':'var(--r)') + '">' + vsPcont + '</td>'
      + '<td>' + tag(inOp?'✓ In range':'Out of range', inOp?'g':'o') + '</td></tr>';
  });

  // Summary note: impact at temperature extremes
  const P_hot  = P_ref * (1 + Math.max(0,(_thi-35)*0.012)) / T_fac;
  const P_cold = P_ref * (1 + Math.abs(Math.min(0,_tlo))*0.015) / T_fac;
  mt += '<tr style="background:var(--bg3);font-size:10px"><td colspan="7" style="padding:6px 10px;color:var(--text2);font-family:var(--mono)">'
    + '&#x26A0; At ' + _thi + '°C max: ' + P_hot.toFixed(1) + ' kW drawn (+'
    + ((P_hot/P_ref-1)*100).toFixed(0) + '% vs 25°C) &nbsp;|&nbsp;'
    + 'At ' + _tlo + '°C min: ' + P_cold.toFixed(1) + ' kW &nbsp;|&nbsp;'
    + (P_electric>0 ? 'From entered loads' : 'Using P_cont=' + (S.t_pcont||50) + ' kW from Project Targets - fill load inputs above for exact values')
    + '</td></tr>';
...
function drawPowerMap() {
  ...
  if (!canvas) return;
  // Store results for dynamic explanation
  window._pmResults = window._pmResults || {};
  ...
  // ── Derating overlay (pv/iv only - mapX is block-scoped to those views) ──
  if (view === 'pv' || view === 'iv') {
    const mapX = v => pad.l + (v-vmin)/(vmax-vmin)*pw;
    const _Pmax_ov = Math.max(...modes.map(m=>m.val))*1.38;
    const mapY = p => pad.t + ph*(1-p/_Pmax_ov);
...
function updatePMExplanation() {
  const el = document.getElementById('pm_result_text');
  if (!el) return;
  const r = window._pmResults || {};
  if (!r.V_nom) {
    el.innerHTML = '<span style="color:var(--text3)">Fill in the input panel - results will appear here after drawing.</span>';
    return;
  }
  const sag_ok  = r.sag_ok !== false;
  const sag_pct = r.V_nom > 0 ? (r.V_sag / r.V_nom * 100).toFixed(1) : '-';
  const v_at_pk = r.V_at_peak ? r.V_at_peak.toFixed(0) : '-';
  const gc = 'color:var(--g)', rc = 'color:var(--r)', bc = 'color:var(--b)', tc = 'color:var(--text)';
  const sagHtml = sag_ok
    ? '<span style="' + gc + '">&#x2713; Sag OK - V at peak: ' + v_at_pk + ' V (above V_min)</span>'
    : '<span style="' + rc + '">&#x2717; Sag too high - V at peak: ' + v_at_pk + ' V (below V_min). Add parallel strings or reduce P_peak.</span>';
  const s_range = document.getElementById('req_s_range') ? document.getElementById('req_s_range').value : '-';
  el.innerHTML =
    '<div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;font-size:12px;color:var(--text2);line-height:1.8">'
    + '<div><b style="' + tc + '">&#x1F4CA; Power Envelope Status</b><br>'
    + 'Pack V_nom: <b style="' + bc + '">' + (r.V_nom||0).toFixed(0) + ' V</b>'
    + ' &nbsp;&#x2022;&nbsp; P_peak: <b style="' + bc + '">' + (r.P_peak||0).toFixed(0) + ' kW</b>'
    + ' &nbsp;&#x2022;&nbsp; P_cont: <b style="' + bc + '">' + (r.P_cont||0).toFixed(0) + ' kW</b><br>'
    + 'I_peak: <b style="' + bc + '">' + (r.I_peak||0).toFixed(0) + ' A</b></div>'
    + '<div><b style="' + tc + '">&#x1F50B; Voltage Sag Under Peak Load</b><br>'
    + 'Sag = ' + (r.V_sag||0).toFixed(1) + ' V (' + sag_pct + '% of V_nom)<br>'
    + sagHtml + '</div>'
    + '<div><b style="' + tc + '">&#x2705; Design Checklist</b><br>'
    + sagHtml + '<br>'
    + '<span>&#x2022; Continuous demand &le; P_cont = unlimited operation<br>'
    + '&#x2022; Peak demand limited to t_peak (Project Targets)</span></div>'
    + '<div><b style="' + tc + '">&#x1F527; If envelope too small</b><br>'
    + '&#x2022; Increase S &rarr; raises V_nom &rarr; lower I<br>'
    + '&#x2022; Increase P &rarr; more capacity &rarr; lower C-rate<br>'
    + '&#x2022; Sizing Calc S range: ' + s_range + '</div>'
    + '</div>';
}
...
function addBOMRow() {
  ...
  tr.style.borderBottom = '1px solid var(--border)';
  tr.innerHTML = `
    <td style="padding:5px 6px"><input type="text" class="bom-name" value="Component" style="width:100%;padding:4px 7px;background:var(--bg3);border:1px solid var(--border);border-radius:4px;color:var(--text);font-size:13px" oninput="calcBOM();try{saveProject()}catch(e){}"></td>
    <td style="padding:5px 6px"><input type="number" class="bom-cost" value="100" step="10" style="width:100%;padding:4px 7px;background:var(--bg3);border:1px solid var(--border);border-radius:4px;color:var(--text);font-family:var(--mono);font-size:13px;text-align:right" oninput="calcBOM();try{saveProject()}catch(e){}"></td>
    <td style="padding:5px 6px"><input type="number" class="bom-qty" value="1" min="1" style="width:100%;padding:4px 7px;background:var(--bg3);border:1px solid var(--border);border-radius:4px;color:var(--text);font-family:var(--mono);font-size:13px;text-align:right" oninput="calcBOM();try{saveProject()}catch(e){}"></td>
...
function calcBOM() {
  // Auto-save all BOM fields to localStorage so they survive refresh
  try { saveProject(); } catch(e) {}
  // BOM fields auto-restored from localStorage via loadProject() on page load
  // Core components
...
window.initApp = function() {
  ...
  setTimeout(() => {
    ...
    // ── STEP 7b: If BT panel is active, render it ──
    setTimeout(() => {
      try {
        const btPanel = document.getElementById('panel-testing');
        if (btPanel && btPanel.classList.contains('active')) {
          if (typeof renderBatteryTesting === 'function') renderBatteryTesting(window._bt_active_cat || 'electrical');
        }
      } catch(e) {}
    }, 500);
...
function runThermalRise() {
  // ── Sync from S object - CORRECTED IR formula (no ×1000 bug) ──
  const autoSync = () => {
    // c_mass is in GRAMS → convert to kg for thermal mass
    const cell_mass_kg = (S.c_mass || 2800) / 1000;
    const n_cells      = (S.c_cps||14) * (S.c_ss||8) * (S.c_pp||1);
    const pack_mass_kg = n_cells * cell_mass_kg + (S.c_housing_mass||50);
    const Cth_kJ       = pack_mass_kg * (S.c_cp_pack||1025) / 1000; // kJ/K
    if (Cth_kJ > 0) setField('tr_Cth', Cth_kJ.toFixed(1));
    // Pack IR in mΩ = cell_ir(mΩ) × S / P  - NO ×1000
    const ir_mOhm = S._packIR_bol > 0
      ? S._packIR_bol
      : (S.c_ir_bol||0.22) * (S.S_total||112) / (S.c_pp||1);
    if (ir_mOhm > 0) setField('tr_ir', ir_mOhm.toFixed(2));
    ...
    if (S.t_top_hi)    setField('tr_Tamb',     S.t_top_hi.toFixed(0));
  };
  try { autoSync(); } catch(e) { console.warn('[TR autoSync]',e); }
...
function processFile(file) {
  ...
  if (name.endsWith('.stl')) {
    ...
    reader.readAsArrayBuffer(file);
  } else if (name.endsWith('.stp') || name.endsWith('.step') || name.endsWith('.std') || name.endsWith('.iges') || name.endsWith('.igs')) {
    // STEP/STP/STD/IGES - cannot be parsed in a static browser app without external server
    setLoading(false);
    showStepGuide(name.split('.').pop().toUpperCase());
  } else {
    showError('Unsupported format. Supported: STL, OBJ, GLTF/GLB.\nFor STEP/STP/STD: export as STL from your CAD tool first.');
    setLoading(false);
  }
}
...
function p3ResetViewer() {
  clearModel();
  const empty = document.getElementById('p3_empty');
  const guide = document.getElementById('p3_step_guide');
  const info  = document.getElementById('p3_file_info');
  if (empty) { empty.style.display = 'flex'; }
  if (guide) { guide.style.display = 'none'; }
  if (info)  { info.textContent = ''; info.style.color = 'var(--g)'; }
  setStatus('');
  setLoading(false);
  // Reset hidden upload inputs so same file can be re-uploaded
  ['p3_hidden_upload','gap_cad_file2'].forEach(id => {
    const el = document.getElementById(id);
    if (el) el.value = '';
  });
}

function showStepGuide(ext, errDetail) {
  setLoading(false);
  const guide = document.getElementById('p3_step_guide');
  const title = document.getElementById('p3_step_ext');
  const empty = document.getElementById('p3_empty');
  if (!guide) { showError(ext + ' format: please export as STL from your CAD tool and re-upload.'); return; }
  if (title) title.textContent = '.' + ext.toLowerCase() + ' file' + (errDetail ? ' - parse failed: ' + errDetail.substring(0,60) : ' detected - conversion required');
  if (empty) empty.style.display = 'none';
  guide.style.display = 'block';
  document.getElementById('p3_file_info').textContent = '⚠ ' + ext + ' - see conversion guide below';
  document.getElementById('p3_file_info').style.color = '#f59e0b';
}
    ...
    <div style="background:#080f1c;border:1px solid rgba(0,212,170,.18);border-radius:20px;padding:44px 44px 36px;width:100%;max-width:400px;box-shadow:0 32px 80px rgba(0,0,0,.8),0 0 24px rgba(0,212,170,.08)">
      ...
      <div style="margin-bottom:20px">
        ...
        <div style="position:relative">
          <input id="login_pw" type="password" placeholder="Enter password" style="width:100%;padding:12px 40px 12px 14px;background:rgba(4,8,15,.8);border:1px solid #1f3856;border-radius:9px;font-size:14px;color:#dde8f8;outline:none;transition:border-color .2s,box-shadow .2s;box-sizing:border-box"
            onfocus="this.style.borderColor='rgba(0,212,170,.5)';this.style.boxShadow='0 0 0 3px rgba(0,212,170,.08)'"
            onblur="this.style.borderColor='#1f3856';this.style.boxShadow='none'"
            onkeydown="if(event.key==='Enter')doLogin()" />
          <button onclick="(function(){var i=document.getElementById('login_pw');var b=document.getElementById('pw_eye');if(i.type==='password'){i.type='text';b.textContent='🙈';}else{i.type='password';b.textContent='👁';}})()" id="pw_eye"
            style="position:absolute;right:10px;top:50%;transform:translateY(-50%);background:none;border:none;cursor:pointer;font-size:16px;color:#3a567a;padding:2px 4px;line-height:1">👁</button>
        </div>
      </div>
...
function onLoginSuccess(session, user) {
  document.body.style.visibility='';
  // Restore saved project data immediately after login
  setTimeout(() => { try { if(loadProject()) { propagate(); } } catch(e) {} }, 200);
