...
(function patchTVR() {
  ...
  window.runTVR = function() {
    ...
    const regen_soc_min = sv('t_regen_soc_margin', 10);
    track(mkRow('Regen headroom @90%SoC', 'Energy', '≥'+regen_soc_min.toFixed(0)+'%', soc_regen<100?`${(100-soc_regen).toFixed(0)}`:'—', ' % SoC margin',
      (100-soc_regen)>=regen_soc_min, soc_regen<100, `DoD=${(sv('t_dod',0.9)*100).toFixed(0)}% → top ${(100-soc_regen).toFixed(0)}% SoC available for regen`), soc_regen<100, soc_regen<=98);
