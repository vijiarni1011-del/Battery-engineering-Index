...
window.renderSizing = function() {
  ...
  /* ── S range: use Project Targets "Cells in Series Range" if set ── */
  ...
  const S_min_v = Math.ceil(t_V_min / c_vmax);
  const S_max_v = Math.floor(t_V_max / (c_vnom * 1.01));
  // If Cells in Series Range is set in Project Targets, use it; else use voltage constraints
  const S_min = req_s_min > 0 ? req_s_min : S_min_v;
  const S_max = req_s_max > 0 ? req_s_max : S_max_v;

  // Always include current Cell Inputs config
  const s_lo = Math.max(1,   Math.min(S_min, curr_Ss > 0 ? curr_Ss : S_min));
  const s_hi = Math.min(300, Math.max(S_max, curr_Ss > 0 ? curr_Ss : S_max));
  const S_range = [];
...
<div class="card" style="margin-bottom:16px">
  ...
  <div style="display:flex;gap:10px;flex-wrap:wrap;margin-top:8px">
    ${req_s_min > 0 && req_s_max > 0 ? `<div style="font-size:11px;color:var(--teal);font-family:var(--mono);margin-bottom:6px">
      🔗 Series range from Project Targets: <b>${req_s_min}–${req_s_max}S</b> · Current config: <b>${curr_cps}×${curr_ss}=${curr_Ss}S/${curr_Pp}P</b>
    </div>` : ''}
    ${[
