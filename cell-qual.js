...
window._buildCertChecks = function(markets, certPrimary) {
  ...
  const stds = [];
  if (certPrimary) stds.push({id:'cert_primary_custom', label:certPrimary, required:true});
  if (mkts.includes('EU') || mkts.includes('CE')) stds.push({id:'cert_eu_iec',  label:'IEC 62619 / 62133 (EU/CE Safety)', required:true});
  if (mkts.includes('EU') || mkts.includes('CE')) stds.push({id:'cert_eu_r100', label:'UNECE R100 Rev.3 (EV Battery)', required:false});
  if (mkts.includes('US')) stds.push({id:'cert_us_ul',   label:'UL 2580 (US Battery Safety)', required:true});
  if (mkts.includes('US')) stds.push({id:'cert_us_fmvss',label:'FMVSS 305 (US EV Powertrain)', required:false});
  if (mkts.includes('CN')) stds.push({id:'cert_cn_gb',   label:'GB 38031-2020 (China EV Battery)', required:true});
  if (mkts.includes('IN')) stds.push({id:'cert_in_ais',  label:'AIS-038 Rev.2 (India)', required:true});
  if (mkts.includes('JP')) stds.push({id:'cert_jp_jis',  label:'JIS C 8712 (Japan)', required:false});
  if (!stds.length) stds.push({id:'cert_general', label:'Enter markets in Project Targets to see standards', required:false});
  return stds.map(s => {
    const checked = document.getElementById(s.id)?.checked || false;
    const reqBadge = s.required ? '<span style="font-size:9px;color:var(--r);margin-left:4px">required</span>' : '';
    return '<label style="display:flex;align-items:center;gap:7px;margin-bottom:5px;cursor:pointer;font-size:11px;color:var(--text2)">'
      + '<input type="checkbox" id="' + s.id + '" ' + (checked?'checked':'') + ' onchange="renderCellQual()">'
      + '<span>' + s.label + reqBadge + '</span></label>';
  }).join('');
};
...
window.renderCellQual = function() {
  ...
  const checks = [
    ...
    {
      // Market certifications — score based on how many required certs are checked
      cat:'Certification', name:`Market Standards (${cell.markets||req.markets})`,
      get _cert_ids() {
        const mkts = (cell.markets||req.markets||'').toUpperCase();
        const ids = ['cert_eu_iec','cert_us_ul','cert_cn_gb','cert_in_ais','cert_primary_custom'].filter(id => document.getElementById(id));
        return ids;
      },
      get _checked() { return this._cert_ids.filter(id => document.getElementById(id)?.checked).length; },
      get _total()   { return Math.max(1, this._cert_ids.length); },
      get val()      { return `${this._checked}/${this._total} confirmed`; },
      get ok()       { return this._checked === this._total; },
      get warn()     { return this._checked >= Math.ceil(this._total/2); },
      target:'All required market standards confirmed',
      note:'Tick the certification checkboxes above as supplier confirms each standard. This feeds the Qualification Radar.',
    },
    {
...
<div class="g2" style="margin-bottom:16px;align-items:stretch">
  ...
  <div class="card">
    ...
    <div style="display:flex;flex-direction:column;gap:8px">
      ...
      <!-- Market-specific certifications — checkboxes for each requirement ──
           These feed directly into the Qualification Radar score -->
      <div style="background:var(--bg4);padding:8px 10px;border-radius:6px;border:1px solid var(--border)">
        <div style="font-size:11px;font-weight:700;color:var(--text);margin-bottom:6px;display:flex;align-items:center;gap:6px">
          Required Certifications for ${cell.markets||req.markets}
          <span style="font-size:9px;font-family:var(--mono);color:var(--teal)">← from Project Targets · tick to confirm supplier compliance</span>
        </div>
        <!-- Derived from markets: show relevant standards as checkboxes -->
${window._buildCertChecks(cell.markets||req.markets||'EU,US', cell.cert_primary)}
      </div>
    </div>
    </div>
  </div>
