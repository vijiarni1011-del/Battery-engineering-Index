...
<div class="g2" style="margin-bottom:16px;align-items:stretch">
  ...
  <div class="card">
    ...
    <div style="display:flex;flex-direction:column;gap:8px">
      ...
      <!-- Market-specific — auto from Project Targets → Certification section -->
      <div style="background:var(--bg4);padding:8px 10px;border-radius:6px;border:1px solid var(--border)">
        <div style="font-size:11px;font-weight:700;color:var(--text);margin-bottom:5px;display:flex;align-items:center;gap:6px">
          Required Certifications for ${cell.markets||req.markets}
          <span style="font-size:9px;font-family:var(--mono);color:var(--teal)">← Project Targets → Certification</span>
        </div>
        ${[
          ['Primary / Safety', cell.cert_primary],
          ['UN 38.3 / Transport', cell.cert_un383_note],
          ['EMC / Emissions', cell.cert_emc],
          ['Other / Regional', cell.cert_other],
        ].filter(([,v])=>v).map(([label,val])=>`
          <div style="display:flex;gap:8px;align-items:flex-start;margin-bottom:3px;font-size:11px">
            <span style="color:var(--text3);min-width:120px;flex-shrink:0">${label}:</span>
            <span style="color:var(--b)">${val}</span>
          </div>`).join('') || '<div style="font-size:11px;color:var(--text3);font-style:italic">Not specified — enter in Project Targets → Certification section</div>'}
      </div>
    </div>
    </div>
  </div>
